 ...
 */
static void
check_str(const string_t *s, const char *expected)
{
  size_t expected_len = strlen(expected);

  // 1. 验证 C-String (strcmp 检查 \0 终止符)
  asrt_msg(strcmp(string_as_cstr(s), expected) == 0, "C-String content mismatch");
...
test_growth(void)
{
  ...
  check_str(&s, "abcdef");
  asrt_msg(s.capacity >= 6, "Capacity did not increase on append_cstr"); // 应该是 8
  ...
  asrt_msg(s.capacity >= 12, "Capacity did not increase on append_slice"); // 应该是 16
...
test_clear(void)
{
  ...
  asrt_msg(s.capacity == old_capacity, "Capacity should be retained after clear");
