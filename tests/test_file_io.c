...
test_write_read_roundtrip(void)
{
  ...
  // a) 验证长度 (不包含 \0)
  asrt_msg(read_slice.len == test_len, "Read length does not match written length");

  // b) 验证内容 (使用 slice_equals_cstr)
  asrt_msg(slice_equals_cstr(read_slice, test_content), "Read content does not match written content");

  // c) 验证 \0 终止符 (read_file_to_slice 的核心特性)
  //    (我们直接使用 strcmp，它依赖 \0)
  asrt_msg(strcmp(read_slice.ptr, test_content) == 0, "Read slice is not a valid C-string (missing \\0?)");
...
test_read_non_existent(void)
{
  ...
  // --- 验证 ---
  asrt_msg(ok == false, "read_file_to_slice should return 'false' for non-existent file");
...
test_write_empty(void)
{
  ...
  asrt_msg(read_slice.ptr[0] == '\0', "Empty file read should still be null-terminated");
