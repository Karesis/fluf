...
test_update_and_safe_get(void)
{
  ...
  // 【【【 修复 】】】
  asrt_msg(strcmp((const char *)strhashmap_get(map, "key"), "value1") == 0, "Get failed after put");

  // 3. 更新
  strhashmap_put(map, "key", VAL("value2"));
  asrt_msg(strhashmap_count(map) == 1, "Count should remain 1 after update");
  // 【【【 修复 】】】
  asrt_msg(strcmp((const char *)strhashmap_get(map, "key"), "value2") == 0, "Value was not updated");
  ...
  // 【【【 修复 】】】
  asrt_msg(strcmp((const char *)out_val, "value2") == 0, "get_ptr returned wrong value");
  ...
  // (这些 NULL 检查是正确的，不需要改)
  asrt_msg(strhashmap_get(map, "null_value") == NULL, "Get for NULL value failed");
...
test_delete(void)
{
  ...
  // 【【【 修复 】】】
  asrt_msg(strcmp((const char *)strhashmap_get(map, "key2"), "val2") == 0, "Key 2 was wrongly deleted");
  ...
  // 【【【 修复 】】】
  asrt_msg(strcmp((const char *)strhashmap_get(map, "key1"), "new_val1") == 0, "Re-insert failed");
...
test_growth(void)
{
  ...
  // 3. 检查所有数据是否仍然存在
  // 【【【 修复 】】】
  asrt_msg(strcmp((const char *)strhashmap_get(map, "key1"), "val1") == 0, "Resize lost key1");
  asrt_msg(strcmp((const char *)strhashmap_get(map, "key2"), "val2") == 0, "Resize lost key2");
  asrt_msg(strcmp((const char *)strhashmap_get(map, "key3"), "val3") == 0, "Resize lost key3");
  asrt_msg(strcmp((const char *)strhashmap_get(map, "key4"), "val4") == 0, "Resize lost key4");

  // 4. 插入更多数据
  strhashmap_put(map, "key5", VAL("val5"));
  asrt_msg(strhashmap_count(map) == 5, "Count should be 5");
  // 【【【 修复 】】】
  asrt_msg(strcmp((const char *)strhashmap_get(map, "key5"), "val5") == 0, "Put after resize failed");
...
test_clear(void)
{
  ...
  asrt_msg(strhashmap_get(map, "key1") == NULL, "Key 1 should be gone after clear");
  asrt_msg(strhashmap_get(map, "key2") == NULL, "Key 2 should be gone after clear");

  // 2. 测试可重用性
  strhashmap_put(map, "key3", VAL("val3"));
  asrt_msg(strhashmap_count(map) == 1, "Count should be 1 after re-put");
  // 【【【 修复 】】】
  asrt_msg(strcmp((const char *)strhashmap_get(map, "key3"), "val3") == 0, "Re-put failed");
