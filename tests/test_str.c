...
TEST(str_iterators)
{
	...
	{
		if (count == 0)
			expect(str_eq_cstr(fruit, "apple"));
		if (count == 1)
			expect(str_eq_cstr(fruit, "banana"));
		if (count == 2)
			expect(str_eq_cstr(fruit, "cherry"));
		count++;
	}
	expect_eq(count, 3);
...
TEST(str_lines_iterator_smart)
{
	...
	{
		if (count == 0)
			expect(str_eq_cstr(line, "A"));
		if (count == 1)
			expect(str_eq_cstr(line, "B"));
		count++;
	}
...
TEST(str_search_ops)
{
	str_t s = str("key=value");
	str_t left, right;

	/// normal case
	expect(str_cut(s, str("="), &left, &right));
	expect(str_eq_cstr(left, "key"));
	expect(str_eq_cstr(right, "value"));
	...
	/// empty separator
	expect(str_cut(s, str(""), &left, &right));
	expect(str_is_empty(left));
	expect(str_eq(right, s));
