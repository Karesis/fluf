...
static void setup_sandbox()
{
	...
	mkdir("sandbox", 0755);
#endif
	asserrt(file_write("sandbox/a.txt", str("content")));
}
...
TEST(fs_type_check)
{
	...
	expect(fs_is_dir("sandbox"));
	expect(!fs_is_file("sandbox"));

	expect(fs_is_file("sandbox/a.txt"));
	expect(!fs_is_dir("sandbox/a.txt"));

	expect(!fs_is_file("non_existent"));
	expect(!fs_is_dir("non_existent"));
