...
static inline utf8_decode_result_t _error_result(void) {
    return (utf8_decode_result_t){ .value = UTF8_REPLACEMENT_CHARACTER, .len = 1 };
}
...
utf8_decode_result_t utf8_decode(const char *ptr, usize len)
{
    ...
    if ((b0 & 0xE0) == 0xC0) {
        if (len < 2) return _error_result();
        
        u8 b1 = data[1];
        if (!_is_continuation(b1)) return _error_result();

        // Overlong check: b0 must be >= 0xC2 for 2-byte sequences
        // (If b0 is C0 or C1, it could represent ASCII, which is illegal here)
        if (b0 < 0xC2) return _error_result();

        rune_t cp = ((rune_t)(b0 & 0x1F) << 6) | 
                    ((rune_t)(b1 & 0x3F));
    ...
    if ((b0 & 0xF0) == 0xE0) {
        if (len < 3) return _error_result();

        u8 b1 = data[1];
        u8 b2 = data[2];
        if (!_is_continuation(b1) || !_is_continuation(b2)) return _error_result();

        // Overlong check:
        // If b0 == E0, b1 must be >= A0
        if (b0 == 0xE0 && b1 < 0xA0) return _error_result();
        
        // Surrogate check (U+D800 .. U+DFFF are forbidden in UTF-8):
        // If b0 == ED, b1 must be < A0 (i.e., up to U+D7FF)
        if (b0 == 0xED && b1 >= 0xA0) return _error_result();
    ...
    if ((b0 & 0xF8) == 0xF0) {
        if (len < 4) return _error_result();

        u8 b1 = data[1];
        u8 b2 = data[2];
        u8 b3 = data[3];
        if (!_is_continuation(b1) || !_is_continuation(b2) || !_is_continuation(b3)) 
            return _error_result();

        // Overlong check:
        // If b0 == F0, b1 must be >= 90
        if (b0 == 0xF0 && b1 < 0x90) return _error_result();
        ...
        if (b0 > 0xF4) return _error_result();
        if (b0 == 0xF4 && b1 >= 0x90) return _error_result();
...
bool utf8_next(utf8_iter_t *it, rune_t *out_cp)
{
    ...
    utf8_decode_result_t res = utf8_decode(it->src.ptr + it->cursor, 
                                           it->src.len - it->cursor);
