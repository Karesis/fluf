...
#include <core/mem/layout.h> // <-- 依赖 layout
...
void bitset_destroy(bitset_t *bs)
{
  ...
  if (bs->words)
  {
    // 使用 fluf 的 allocer_free
    layout_t layout = layout_of_array(uint64_t, bs->num_words);
    allocer_free(bs->alc, bs->words, layout);
  }
  memset(bs, 0, sizeof(bitset_t)); // 清理
}
...
{
  asrt_msg(dest->num_bits == src1->num_bits && src1->num_bits == src2->num_bits, "Bitset op size mismatch");
...
size_t
bitset_count(const bitset_t *bs)
{
  // (重构为更简单、更快的版本)
  // (既然你只用 Clang, 我们可以安全地使用 __builtin_popcountll)

  size_t count = 0;
  if (bs->num_words == 0)
    return 0;

  // 1. 计算所有*完整* word 的 popcount
  ...
  // 2. 计算*最后一个* word (它可能不是完整的)
  uint64_t last_word = bs->words[bs->num_words - 1];
  size_t remaining_bits = bs->num_bits % 64;

  if (remaining_bits == 0)
  {
    // (num_bits 是 64 的倍数, 最后一个 word 是完整的)
    count += __builtin_popcountll(last_word);
  }
  else
  {
    // (num_bits 不是 64 的倍数, 我们必须屏蔽掉多余的位)
