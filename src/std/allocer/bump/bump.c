...
static void aligned_free_internal(void *ptr) { _aligned_free(ptr); }
#else
...
static void aligned_free_internal(void *ptr) { free(ptr); }
#endif
...
static size_t round_up_to(size_t n, size_t divisor) {
  asrt(is_power_of_two(divisor));
  return (n + divisor - 1) & ~(divisor - 1);
}

static uintptr_t round_down_to(uintptr_t n, size_t divisor) {
  asrt(is_power_of_two(divisor));
  return n & ~(divisor - 1);
}
...
static chunkfooter_t *get_empty_chunk() {
...
static bool chunk_is_empty(chunkfooter_t *footer) {
...
static void dealloc_chunk_list(chunkfooter_t *footer) {
...
static chunkfooter_t *new_chunk(bump_t *bump, size_t new_size_without_footer,
                                size_t align, chunkfooter_t *prev) {

  new_size_without_footer = round_up_to(new_size_without_footer, CHUNK_ALIGN);

  size_t alloc_size;
  if (__builtin_add_overflow(new_size_without_footer, FOOTER_SIZE,
                             &alloc_size)) {
    return NULL;
  }
  ...
  unsigned char *data =
      (unsigned char *)aligned_malloc_internal(align, alloc_size);
  if (!data)
    return NULL;
...
static void *alloc_layout_slow(bump_t *bump, layout_t layout) {
  ...
  size_t requested_align =
      (layout.align > bump->min_align) ? layout.align : bump->min_align;
  ...
  size_t chunk_align =
      (layout.align > CHUNK_ALIGN) ? layout.align : CHUNK_ALIGN;
  chunk_align = (chunk_align > bump->min_align) ? chunk_align : bump->min_align;

  chunkfooter_t *new_footer =
      new_chunk(bump, new_size_without_footer, chunk_align, current_footer);
  if (!new_footer) {
    return NULL;
  }
  ...
  if (layout.align <= bump->min_align) {
    ...
    unsigned char *aligned_ptr_end =
        (unsigned char *)round_down_to((uintptr_t)ptr, layout.align);
...
static void *try_alloc_layout_fast(bump_t *bump, layout_t layout) {
  ...
  asrt_msg((chunk_is_empty(footer) || ((uintptr_t)ptr % min_align) == 0),
           "bump_t pointer invariant broken");
...
void bump_init_with_min_align(bump_t *bump, size_t min_align) {
  ...
  asrt_msg(min_align <= CHUNK_ALIGN,
...
void bump_init(bump_t *bump) { bump_init_with_min_align(bump, 1); }
...
bump_t *bump_new(void) { return bump_new_with_min_align(1); }
...
void bump_reset(bump_t *bump) {
  ...
  current_footer->ptr =
      (unsigned char *)round_down_to(ptr_start, bump->min_align);

  size_t usable_size =
      (size_t)((unsigned char *)current_footer - current_footer->data);
...
void *bump_alloc_copy(bump_t *bump, const void *src, size_t size,
                      size_t align) {
  if (size == 0) {
